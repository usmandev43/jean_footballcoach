          ...
          duration:(NSTimeInterval)duration
{
    ...
    if(mainNotes!=nil)
    {
        [mainNotes setVisible:NO animated:YES completion:nil];
    }
    ...
    if (self) {
        ...
        //Content views
        {
            //textLabel
            {
                ...
                _dismissButton = [[UIButton alloc] initWithFrame:CGRectMake(UIScreen.mainScreen.bounds.size.width-35,barHeight, 30, 30)];
                [_dismissButton setImage:[UIImage imageNamed:@"thincross.png"] forState:UIControlStateNormal];
                ...
                //[_textLabel addSubview:_dismissButton];
...
- (CGRect)visibleFrame
{
    ...
    NSLog(@"%f",viewController.view.frame.size.width);
    ...
    [_dismissButton setFrame:CGRectMake(UIScreen.mainScreen.bounds.size.width-28,displayFrame.size.height-40, 30, 30)];
    return displayFrame;
}
...
- (void)updateSymbolView
{
    ...
    NSLog(@"%f",_symbolView.frame.size.width);
...
+ (UIColor*)blurTintColorForStyle:(CSNotificationViewStyle)style
{
    ...
    switch (style) {
        case CSNotificationViewStyleSuccess:
            blurTintColor = [UIColor greenColor];
            break;
        case CSNotificationViewStyleError:
